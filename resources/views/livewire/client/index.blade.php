<section class="w-full">
    ...
    <div class="flex flex-col xl:justify-between xl:flex-row my-6">
        <div class="mb-6">
            ...
            <flux:subheading size="lg">View and manage all clients.</flux:subheading>
        </div>
        <div class="flex items-top gap-2.5">
            @can('admin.clients.import')
            ...
            @can('admin.clients.create')
                <flux:button variant="primary" :href="route('clients.create')" icon="plus">{{ __('Add New') }}</flux:button>
            @endcan
        </div>
    </div>
                        ...
                        <div class="inline-block min-w-full py-2 align-middle">
                            <table class="w-full divide-y divide-zinc-300 dark:divide-zinc-600">
                                ...
                                @foreach ($clients as $client)
                                    <tr class="hover:bg-gray-50 hover:dark:bg-zinc-700" wire:key="{{ $client->id }}">
                                        											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $client->name }}</flux:text></td>
											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $client->email }}</flux:text></td>
											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $client->phone }}</flux:text></td>
											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $client->address }}</flux:text></td>
											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $client->notes }}</flux:text></td>

                                        <td class="whitespace-nowrap px-3 py-4 flex gap-1 text-sm font-medium text-gray-900">
                                            @can('admin.clients.show')
                                                <flux:button variant="ghost" href="{{ route('clients.show', $client->id) }}" icon="magnifying-glass" class="mr-2" size="xs" tooltip="View details">
                                                </flux:button>
                                            @endcan
                                            @can('admin.clients.edit')
                                                <flux:button variant="ghost" href="{{ route('clients.edit', $client->id) }}" icon="pencil-square" class="mr-2" size="xs" tooltip="Edit">
                                                </flux:button>
                                            @endcan
                                            @can('admin.clients.delete')
                                                ...
                                                <flux:modal :name="'delete-client-'.$client->id" class="min-w-[22rem]">
                                                    <div class="space-y-6">
                                                        <div>
                                                            <flux:heading size="lg">Delete {{ $client->id }}?</flux:heading>
                                                        ...
                                                        <div class="flex gap-2">
                                                            ...
                                                            <flux:button type="button" variant="danger" wire:click="delete({{ $client->id }})">Delete</flux:button>
                                                        </div>
                                                    </div>
                                                </flux:modal>
                                            @endcan
