<section class="w-full">
    ...
    <div class="mb-6">
        <flux:breadcrumbs>
            ...
            <flux:breadcrumbs.item>Branches</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>
    ...
    <div class="flex flex-col xl:justify-between xl:flex-row my-6">
        <div class="mb-6">
            ...
            <flux:subheading size="lg">View and manage all branches.</flux:subheading>
        </div>
        <div class="flex items-top gap-2.5">
            @can('admin.branches.import')
            ...
            @can('admin.branches.create')
                <flux:button variant="primary" :href="route('branches.create')" icon="plus">{{ __('Add New') }}</flux:button>
            @endcan
        </div>
    </div>
                    ...
                    <div class="overflow-x-auto">
                        <div class="mx-2 mt-2 mb-4 flex flex-col md:flex-row gap-3 justify-between">
                            <div class="w-full max-w-xs">
                                <flux:input wire:model.live="search" icon="magnifying-glass" placeholder="Search branches"/>
                            </div>
                            <div>
                        ...
                        <div class="inline-block min-w-full py-2 align-middle">
                            <table class="w-full divide-y divide-zinc-300 dark:divide-zinc-600">
                                ...
                                <tr>
										...
										<th scope="col" class="px-3 py-3 text-left text-xs font-semibold uppercase tracking-wide"><flux:text variant="subtle" class="text-xs">Contact Number</flux:text></th>
                                ...
                                @foreach ($branches as $branch)
                                    <tr class="hover:bg-gray-50 hover:dark:bg-zinc-700" wire:key="{{ $branch->id }}">
                                        											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $branch->name }}</flux:text></td>
											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $branch->contact_number }}</flux:text></td>
											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $branch->address }}</flux:text></td>
											<td class="whitespace-nowrap px-3 py-4 text-sm"><flux:text>{{ $branch->description }}</flux:text></td>

                                        <td class="whitespace-nowrap px-3 py-4 flex gap-1 text-sm font-medium text-gray-900">
                                            @can('admin.branches.show')
                                                <flux:button variant="ghost" href="{{ route('branches.show', $branch->id) }}" icon="magnifying-glass" class="mr-2" size="xs" tooltip="View details">
                                                </flux:button>
                                            @endcan
                                            @can('admin.branches.edit')
                                                <flux:button variant="ghost" href="{{ route('branches.edit', $branch->id) }}" icon="pencil-square" class="mr-2" size="xs" tooltip="Edit">
                                                </flux:button>
                                            @endcan
                                            @can('admin.branches.delete')
                                                <flux:modal.trigger name="delete-branch-{{ $branch->id }}">
                                                    <flux:button variant="ghost" icon="trash" class="text-red-600!" size="xs" tooltip="Delete"></flux:button>
                                                </flux:modal.trigger>

                                                <flux:modal :name="'delete-branch-'.$branch->id" class="min-w-[22rem]">
                                                    <div class="space-y-6">
                                                        <div>
                                                            <flux:heading size="lg">Delete {{ $branch->id }}?</flux:heading>
                                                        ...
                                                        <div class="flex gap-2">
                                                            ...
                                                            <flux:button type="button" variant="danger" wire:click="delete({{ $branch->id }})">Delete</flux:button>
                                                        </div>
                                                    </div>
                                                </flux:modal>
                                            @endcan
